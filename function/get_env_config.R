...
get_env_config <- function(MAC = FALSE) {
  ...
  if (MAC) {
    ...
    list(
      ...
      # Use osascript to trigger native macOS mounting. 
      # This bypasses the need to manually mkdir in /Volumes.
      mount_cmd           = sprintf("osascript -e 'mount volume \"smb://guest:@%s/fileShare\"'", server_ip),
      # bwlimit             = "20M", # not sure this is needed so keeping off for now
